...
files = c("results_053")
ks= c(6)
estimMu.s=c(0.4,0.5,0.4,0.4,0.4,0.4,0.4,0.5)
alphas=c(2/3,2/3,2/3,2/3,2/3,2/3,2/3,2/3)
generations=c(4,2,4,4,3,3,4,2)


file.path="/Users/alejandrog/MEGA/Caltech/trees/GraceData/10mer/"
fileName=files[file.idx]
fileName=paste(file.path,fileName,sep="")
k=ks[file.idx]
estimMu = estimMu.s[file.idx]
estimAlpha=alphas[file.idx]
estimG =generations[file.idx]
...
#read csv as dataframe
 posInfo=read.csv(paste(fileName,".csv",sep=""),sep="\t")
...
avgEditRate_r = array()
avgEditRate_x = array()
nCells_ =  length(posInfo$Summary)
for(i in 1:nCells_){
  avgEditRate[i] = sum(strsplit(toString(posInfo$Summary[i]),"")[[1]]=="u")/lengthBarcode
  avgEditRate_r[i] = sum(strsplit(toString(posInfo$Summary[i]),"")[[1]]=="r")/lengthBarcode
  avgEditRate_x[i] = sum(strsplit(toString(posInfo$Summary[i]),"")[[1]]=="x")/lengthBarcode
}
estimMu = 1-mean(avgEditRate)^(1/4)
estimAlpha = 1-mean(avgEditRate_x / (avgEditRate_r + avgEditRate_x))
 #features are in
...
dend<-hclust(as.dist(t(matdist_)))
