...
#erase chace files
system("rm /Users/alejandrog/MEGA/Caltech/trees/GraceData/10mer_membow_2019/editRate/*")
...
file.path="/Users/alejandrog/MEGA/Caltech/trees/GraceData/10mer_membow_2019/"
...
existing.files = list.files("../GraceData/10mer_membow_2019/")
#read the xls with all the files info (some are not in the folder)
all.files=read.csv(paste(file.path,"results.guide.membow.2019.csv",sep=""))
       ...
       #which will be constant. We do this because some calculations of base frequency need all actg bases to be present.
        ...
        if(clust.method==1){
          ...
          if(length(barcodes)>4){
            d3.list = normalized.RF.membow(true.tree,manualTree,barcodes,posInfo)
            d3 =d3.list[[1]]
            n.colony=d3.list[[2]]
          ...
          this.score = t( c (d1,d2,d3))
          this.score = t(c(toString(all.files$file.name[file.idx]),d1,d3,n.colony,d4))
...
read.files <- function (file.path.membow,file.path.memoir){

  rate.dist.file.membow = paste(file.path.membow,"editRate/distribution_RF_perTree.txt",sep="",
  collapse="");
  rate.dist.file.memoir = paste(file.path.memoir,"editRate/distribution_RF_perTree.txt",sep="",
  collapse="");

  rate.dist.membow = read.table(rate.dist.file.membow)
  rate.dist.memoir = read.table(rate.dist.file.memoir)

  return(list(rate.dist.membow,rate.dist.memoir))
}
...
compare.ecdf<-function(file.path){
  ...
  for(i in 2:length(names(rate.dist) )) {
    a = sort(rate.dist[,i])
    if(i ==2){
      plot(a,(1:n)/n,type = "l",col = "black",
      ylab = "RF score",xlab="Fraction of colonies",main="MEMbow scores all colonies",
    lwd = 2)
    }else {
      lines(a,(1:n)/n,lwd=2,col="red")
...
compare.ecdf2<- function(rate.dist){

  metrics = c("Full RF","Clade RF")
  rate.dist= rate.dist[rate.dist$V3>0, ]
    n = dim(rate.dist)[1]
  rate.dist.v2  =rate.dist[order(rate.dist$V2),]
  rate.dist.v3  =rate.dist[order(rate.dist$V3),]
  df= data.frame(RF=c(rate.dist.v2$V2,rate.dist.v3$V3)   ,
    probs =c( (1:n)/n, (1:n)/n),metric = factor(c(rep_len(metrics[1],n),rep_len(metrics[2],n))))

    ggplot(data = df, aes(x = RF,y=probs,group = metric ,color=metric) ) +
      geom_line(size = 1.2) + geom_point() +
      theme(text = element_text(size=21)) + labs(x = "Tree score (RF)",y="Fraction of colonies") +
      theme_minimal(base_size = 20) + xlim(0, 1)  + ylim(0,1)
}

file.path.memoir ="/Users/alejandrog/MEGA/Caltech/trees/GraceData/10mer/"
file.path.membow = file.path

compare.ecdf.memoir.membow<- function(file.path.membow,file.path.memoir){
  ...
  rate.dist.membow = rate.dist.membow[rate.dist.membow$V3>0, ]

  n.membow = dim(rate.dist.membow)[1]
  n.memoir = dim(rate.dist.memoir)[1]

  metrics = c("MEMbow","MEMOIR")
  rate.dist.v2  =rate.dist.memoir[order(rate.dist.memoir$V2),]
  rate.dist.v3  =rate.dist.membow[order(rate.dist.membow$V3),]

  df= data.frame(RF=c(rate.dist.v3$V3,rate.dist.v2$V2)   ,
    probs =c( (1:n.membow)/n.membow, (1:n.memoir)/n.memoir),metric = factor(c(rep_len(metrics[1],n.membow),rep_len(metrics[2],n.memoir))))
...
#boxplot for comparing RF scores for MEMbow and MEMOIR trees
#Oct 21th 2018
compare.hist.memoir.membow<-function (file.path.membow,file.path.memoir){
  ...
  ggplot(df,aes(metric,RF,fill=metric)) + geom_boxplot(outlier.size=0) + scale_fill_manual(values=wes_palette(n=2, name="GrandBudapest2")) +
  geom_jitter(aes(metric,RF), position = position_jitter(width=0.1,height=0),
              alpha=0.6,size=3,show_guide =F) +
  xlab("Recording mode") +
  ylab("Reconstructability") +
...
n.vs.rf<-function(file.path.membow,file.path.memoir,bins){

  results = read.files(file.path.membow,file.path.memoir)
  rate.dist.membow = results[[1]]
  rate.dist.memoir = results[[2]]

  x = c(rate.dist.membow$V4[rate.dist.membow$V3>0],rate.dist.memoir$V3)
  y = c(rate.dist.membow$V3[rate.dist.membow$V3>0],rate.dist.memoir$V2)
  dat = data.frame(x = x,y = y)
  p=ggplot(dat,aes(x=x,y=y)) + geom_point() + theme(legend.position = "none") + xlab("Colony size") +
  ylab("Reconstructability") +

    theme(text = element_text(size=21))


  ggMarginal(p,type="histogram",fill="cyan4",xparams=list(bins=10),yparams = list(bins=bins))
